...
1. 拒绝"假通"测试：严禁仅用 curl 测试 API。必须通过 MCP 或 browser-use 调用真实浏览器进行端到端点击测试。
2. 原生视觉走查：截图后直接调用AI原生视觉能力"看"截图判断 UI 问题。
3. 带状态调试：使用 browser-use --browser real --profile "Default" 挂载本地真实 Chrome。
4. Git 存档回滚：错误时自动 git reset --hard 回滚。
5. 原子化任务粒度：task.json 必须拆解到最小独立任务。
6. 上下文"无状态"化：强制通过文件读取获取状态，不依赖长对话记忆。
7. 文件权限隔离：Backend Agent 只能写 /api，Frontend Agent 只能写 /src。

升级方向：RAG 动态知识库、成本熔断机制、Human-in-the-Loop 网关
```
